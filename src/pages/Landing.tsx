          ...
          >
            {stats.map((stat, index) => (
              <Card 
                ...
                className={`${index === 1 ? 'bg-yellow-400 text-black' : 'bg-white/5 text-white'} backdrop-blur-sm rounded-xl p-6 text-center border border-white/10`}
              >
                <CardContent devId="noID"  className="p-0">
                  <Div devId="noID" className={`text-2xl font-bold mb-2 ${index === 1 ? 'text-black' : 'text-white'}`}>{stat.value}</Div>
                  <Div devId="noID" className={`${index === 1 ? 'text-black' : 'text-gray-400'}`}>{stat.label}</Div>
                </CardContent>
              </Card>
            ))}
          </Div>
